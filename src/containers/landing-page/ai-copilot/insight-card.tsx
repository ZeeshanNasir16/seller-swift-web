...
const InsightCard = ({
  ...
  return (
    <div
      className='rounded-lg relative z-20 p-10 lg:p-16 pt-16 text-center'
      style={{
        background:
    ...
    >
      <Image
        ...
        height={0}
        sizes='100%'
        className='w-20 h-20 object-contain absolute -top-10 left-1/2 -translate-x-1/2'
        loading='lazy'
      />

      <h3 className='text-2xl lg:text-3xl font-bold mb-3'>{title}</h3>
      <p className='text-sm font-light mb-6'>{description}</p>
      <p className='text-sm font-light italic'>{tagline}</p>
    </div>
  );
};
